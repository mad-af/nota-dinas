      ...
      <div class="mx-auto space-y-6 max-w-7xl sm:px-6 lg:px-6">
        <div class="p-6 bg-white shadow-sm sm:rounded-lg">
          <div class="flex flex-wrap gap-3 justify-between items-center">
            ...
            <div class="flex flex-wrap gap-2 items-center">
              <input v-model="search" @keyup.enter="applyFilters" type="text" placeholder="Cari endpoint/correlation/error" class="px-2 py-1 w-64 text-sm rounded border" />
        ...
        <div class="p-0 bg-white shadow-sm sm:rounded-lg">
            ...
            <table class="min-w-full text-sm">
              <thead>
                <tr class="bg-gray-50 text-gray-700">
                  ...
                  <th class="px-4 py-2 text-left">Correlation</th>
              ...
              <tbody>
                <tr v-for="item in (props.logs?.data || [])" :key="item.id" class="border-t">
                  ...
                  <td class="px-4 py-2 text-gray-700">{{ item.correlation_id }}</td>
                  <td class="px-4 py-2 text-gray-700">{{ item.user_id || '-' }}</td>
                  <td class="px-4 py-2 text-gray-700">{{ item.endpoint }}</td>
                  <td class="px-4 py-2 text-gray-700">{{ item.method }}</td>
                  <td class="px-4 py-2" :class="item.status_code >= 400 ? 'text-red-600' : 'text-green-600'">{{ item.status_code }}</td>
                  <td class="px-4 py-2 text-gray-700">{{ item.duration_ms }} ms</td>
                  <td class="px-4 py-2 text-gray-700">{{ item.error_message || '-' }}</td>
                  <td class="px-4 py-2">
                    <button @click="openDetail(item)" class="px-3 py-1 text-xs bg-gray-100 rounded hover:bg-gray-200">Detail</button>
                  </td>
                </tr>
          ...
          <div class="flex justify-between items-center px-4 py-3">
            <div class="text-xs text-gray-500">Total: {{ props.logs?.total || 0 }} • Halaman {{ props.logs?.current_page || 1 }}/{{ props.logs?.last_page || 1 }}</div>
            <div class="flex gap-2">
              <button :disabled="!(props.logs?.prev_page_url)" @click="router.visit(props.logs?.prev_page_url)" class="px-3 py-1 text-xs rounded border disabled:opacity-50">Prev</button>
              <button :disabled="!(props.logs?.next_page_url)" @click="router.visit(props.logs?.next_page_url)" class="px-3 py-1 text-xs rounded border disabled:opacity-50">Next</button>
            </div>
          </div>
        </div>
        ...
        <Modal :show="showDetail" @close="closeDetail">
          <div class="p-6 w-full max-w-2xl bg-white rounded-lg shadow-lg">
            ...
            <div class="grid grid-cols-2 gap-3 text-sm mb-4">
              <div><span class="text-gray-500">Correlation:</span> {{ detail?.correlation_id }}</div>
              <div><span class="text-gray-500">User:</span> {{ detail?.user_id || '-' }}</div>
            ...
            <div class="mb-3">
              ...
              <pre class="p-3 bg-gray-900 text-gray-100 rounded overflow-auto text-xs">{{ JSON.stringify(detail?.request_payload, null, 2) }}</pre>
            </div>
            <div>
            ...
            <div class="flex justify-end mt-4">
              <button @click="closeDetail" class="px-3 py-2 text-xs bg-gray-100 rounded sm:text-sm hover:bg-gray-200">Tutup</button>
            </div>
          </div>
        </Modal>
      </div>
