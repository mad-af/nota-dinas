...
<template>
  ...
  <div class="min-h-screen bg-gray-50">
    <div class="sticky top-0 z-10 py-2 w-full text-sm text-center text-white bg-indigo-600">Tampilan Publik</div>
    <div class="px-2 sm:px-4 lg:px-6">
      <div class="py-4 mx-auto max-w-5xl sm:py-8">
        <div class="bg-white rounded-md shadow-sm">
          <div class="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 class="text-base font-semibold text-gray-800 sm:text-lg">{{ doc?.name || 'Dokumen' }}</h1>
              <p class="text-xs text-gray-500 sm:text-sm">Status: {{ hasSigned ? 'Sudah TTE' : 'Original' }}</p>
            </div>
            <div class="flex gap-2 items-center">
              <a :href="doc?.url" download
                class="px-4 py-2 text-sm font-medium text-white bg-green-600 rounded hover:bg-green-700">Download</a>
              <button @click="prevPage" class="px-3 py-2 text-sm bg-gray-100 rounded hover:bg-gray-200">Prev</button>
              <span class="text-xs text-gray-700 sm:text-sm">Halaman {{ currentPage }} / {{ totalPages || '-' }}</span>
              <button @click="nextPage" class="px-3 py-2 text-sm bg-gray-100 rounded hover:bg-gray-200">Next</button>
            </div>
          </div>
            ...
            <div v-else class="w-full">
              <div class="relative w-full">
                <div ref="wrapRef" class="relative w-full bg-gray-50">
                  ...
                  <div
                    class="absolute top-2 right-2 px-2 py-1 text-xs text-white rounded pointer-events-none bg-indigo-600/80">
                    Publik</div>
                </div>
              </div>
            </div>
